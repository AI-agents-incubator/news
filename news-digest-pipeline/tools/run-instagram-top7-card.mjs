...
// Render a review-only seven-item card from an explicit local digest. This
// tool never imports the autoposter, credentials, Graph API, or a language model.
...
function usage() {
  return 'Usage: node tools/run-instagram-top7-card.mjs --input <digest.txt> --run-id <immutable-id>';
}
...
async function main() {
  ...
  const entries = extractTop7Headlines(digestText);
  const input = {
  ...
  const extracted = {
    schema_version: 'instagram-top7-white-card-lab.extracted-headlines.v1',
  ...
  try {
    const rendered = await renderTop7Card(entries, join(runDir, 'preview-1080x1350.png'));
    ...
    await writeFile(join(runDir, 'manifest.json'), JSON.stringify({
      schema_version: 'instagram-top7-white-card-lab.run.v1',
      run_id: runId,
      status: 'succeeded',
      input,
      extracted: {
...
main().catch((error) => {
  console.error(`top7 white-card run failed: ${String(error?.message || error)}`);
  process.exitCode = 1;
});
